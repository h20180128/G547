...
static const unsigned short probe_i2c_addrs[] = I2C_ADDRS(0x48, 0x49);
...
static int __init lm75_probe_init(void)
{
	int nr;
...
static void __exit lm75_probe_exit(void)
{
	printk(KERN_ALERT "%s\n", __FUNCTION__);
}

module_init(lm75_probe_init);
module_exit(lm75_probe_exit);

MODULE_AUTHOR("Borislav Sapundzhiev <BSapundzhiev@gmail.com>");
MODULE_DESCRIPTION("lm75 init");
MODULE_LICENSE("GPL");
