...
{
	int ret=0;
	printk(KERN_INFO "dordrive : open \n");
	//goto out;
...
static void my_release(struct gendisk *disk, fmode_t mode)
{
	...
	printk(KERN_INFO "dordrive : closed \n");
...
static int rb_transfer(struct request *req)
{
	...
	rq_for_each_segment(bv, req, iter)
	{
		...
		printk(KERN_DEBUG "dor: Start Sector: %llu, Sector Offset: %llu;\
...
{
	mydisk_init();
	c = register_blkdev(c, "dor");// major no. allocation
	...
	sprintf(((device.gd)->disk_name), "dor");
...
{	
	...
	printk(KERN_ALERT "dordrive is unregistered");
